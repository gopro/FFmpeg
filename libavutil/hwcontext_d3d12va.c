...
static PFN_D3D12_CREATE_DEVICE mD3D12CreateDevice;
...
static av_cold void load_functions(void)
{
    ...
    HANDLE d3d11lib, d3d11on12lib, d3d12lib, dxgilib;

    d3d11lib = dlopen("d3d11.dll", 0);
    d3d11on12lib = dlopen("d3d11on12.dll", 0);
    d3d12lib = dlopen("d3d11on12.dll", 0);
    dxgilib = dlopen("dxgi.dll", 0);

    if (!d3d11lib || !d3d11on12lib || !d3d12lib || !dxgilib)
        return;

    mD3D12CreateDevice = (PFN_D3D12_CREATE_DEVICE) GetProcAddress(d3d12lib, "D3D12CreateDevice");
    mCreateDXGIFactory = (PFN_CREATE_DXGI_FACTORY) GetProcAddress(dxgilib, "CreateDXGIFactory2");
#else
    // In UWP (which lacks LoadLibrary), CreateDXGIFactory isn't available,
    // only CreateDXGIFactory1
    mD3D12CreateDevice = (PFN_D3D11_CREATE_DEVICE) D3D12CreateDevice;
...
static AVBufferRef *d3d12va_alloc_single(AVHWFramesContext *ctx)
{
    D3D12VAFramesContext       *s = ctx->internal->priv;
...
static AVBufferRef *d3d12va_pool_alloc(void *opaque, size_t size)
{
    ...
    if (!hwctx->texture)
        return d3d12va_alloc_single(ctx);
...
static int d3d12va_create_staging_texture(AVHWFramesContext *ctx, DXGI_FORMAT format)
{
    AVD3D12VADeviceContext *device_hwctx = ctx->device_ctx->hwctx;
    D3D12VAFramesContext              *s = ctx->internal->priv;
    HRESULT hr;
                                 ...
                                 const AVFrame *src)
{
    ...
    ID3D12Resource *texture = (ID3D12Resource *)(ID3D11Texture2D *)frame->data[0];
    ...
    if (!s->staging_texture) {
        ...
        int res = d3d12va_create_staging_texture(ctx, desc.Format);
        if (res < 0)
            return res;
    }

    staging = (ID3D12Resource *)s->staging_texture;
...
static int d3d12va_device_init(AVHWDeviceContext *hwdev)
{
    ...
    if (!device_hwctx->lock) {
        ...
        device_hwctx->lock   = d3d12va_default_lock;
        device_hwctx->unlock = d3d12va_default_unlock;
    }
                                 ...
                                 AVDictionary *opts, int flags)
{
    ...
    hr = mD3D12CreateDevice(pAdapter, pAdapter ? D3D_DRIVER_TYPE_UNKNOWN : D3D_DRIVER_TYPE_HARDWARE, NULL, creationFlags, NULL, 0,
    ...
    hr = ID3D12Device_QueryInterface(device_hwctx->device, &IID_ID3D11Multithread, (void **)&pMultithread);
    if (SUCCEEDED(hr)) {
