...
static IMFSample *mf_avpacket_to_sample(AVCodecContext *avctx, const AVPacket *avpkt)
{
    ...
    sample = ff_create_memory_sample(&mfapi_funcs, tmp.data, tmp.size, c->in_info.cbAlignment);
    if (sample) {
...
static int mf_deca_output_type_get(AVCodecContext *avctx, IMFMediaType *type)
{
    ...
    if (FAILED(hr))
        return AVERROR_EXTERNAL;
    avctx->channels = t;
    avctx->channel_layout = av_get_default_channel_layout(t);  //RFJ Solve Deprecated
...
static IMFSample *mf_a_avframe_to_sample(AVCodecContext *avctx, const AVFrame *frame)
{
    ...
    sample = ff_create_memory_sample(&mfapi_funcs, frame->data[0], len, c->in_info.cbAlignment);
    if (sample)
...
static IMFSample *mf_v_avframe_to_sample(AVCodecContext *avctx, const AVFrame *frame)
{
    ...
    sample = ff_create_memory_sample(&mfapi_funcs, NULL, size, c->in_info.cbAlignment);
    if (!sample)
        return NULL;
    ...
    while (1) {
        *out_sample = NULL;
        ...
        if (!c->out_stream_provides_samples) {
            sample = ff_create_memory_sample(&mfapi_funcs, NULL, c->out_info.cbSize, c->out_info.cbAlignment);
            if (!sample)
                return AVERROR(ENOMEM);
        }
...
static void mf_flush(AVCodecContext *avctx)
{
    ...
    if (c->mfa.async_events) {
        hr = IMFMediaEventGenerator_QueueEvent(c->mfa.async_events, ff_METransformMarker,
                                               &GUID_NULL, S_OK, NULL);
        if (FAILED(hr)) {
            av_log(avctx, AV_LOG_ERROR, "sending marker failed\n");
        } else {
            while (!c->mfa.async_marker) {
                if (mf_wait_events(avctx) < 0)
                    break; // just don't lock up
                c->mfa.async_need_input = c->mfa.async_have_output = c->draining_done = 0;
            }
            c->mfa.async_marker = 0;
        }
    }
    ...
    c->sample_sent = 0;
    c->draining_done = 0;
    c->mfa.async_need_input = c->mfa.async_have_output = 0;
